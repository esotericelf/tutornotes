...
-- Wait for profiles table to be available before creating policies
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'profiles' AND table_schema = 'public') THEN
        RAISE NOTICE 'Waiting for profiles table to be created...';
        -- This will pause execution until profiles table exists
        PERFORM pg_sleep(1);
    END IF;
END $$;
