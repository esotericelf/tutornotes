/**
 * Unified Analytics Service for Math Papers
 * Provides comprehensive analytics and statistics with caching
 */
...
class UnifiedAnalyticsService extends BaseService {
    ...
    static async getTagAnalytics(tag) {
        try {
            ...
            // Check cache first
            const cached = TagCacheService.getCachedTagAnalytics(trimmedTag);
            if (cached) {
                return {
                    data: cached,
                    error: null,
            ...
            console.log('📊 Getting analytics for tag:', trimmedTag);

            // Get basic tag statistics
            const statsResult = await UnifiedTagService.getTagStatistics(trimmedTag);
            if (statsResult.error) {
                console.error('Error getting tag statistics:', statsResult.error);
                return { ...statsResult, fromCache: false };
            }
            ...
            if (!stats) {
                return {
                    data: null,
                    error: new Error(`No data found for tag: ${trimmedTag}`),
                    fromCache: false
                };
            }

            // Get papers for this tag to analyze patterns
            const papersResult = await UnifiedTagService.getMathPapersByTags([trimmedTag], 100);
            if (papersResult.error) {
                console.error('Error getting papers for analytics:', papersResult.error);
                return { ...papersResult, fromCache: false };
            }
            ...
            const analytics = {
                ...
                analytics: {
                    ...
                    tagCoOccurrence: this.analyzeTagCoOccurrence(papers, trimmedTag),
                ...
                // URL information
                url: UnifiedURLService.createURLWithTags([trimmedTag]),
                encodedTag: UnifiedURLService.encodeTagsForURL([trimmedTag])
            };

            // Cache the results
            TagCacheService.cacheTagAnalytics(trimmedTag, analytics);
